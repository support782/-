...
import { Upload, CheckCircle, XCircle, Loader2, FileText, User, Camera } from 'lucide-react';
...
export default function KYC() {
  ...
  const handleVerifyWithAI = async () => {
    ...
    try {
      ...
      const images = [
        { inlineData: { data: files.nidFront.split(',')[1], mimeType: 'image/jpeg' } },
        { inlineData: { data: files.nidBack.split(',')[1], mimeType: 'image/jpeg' } },
        { inlineData: { data: files.selfie.split(',')[1], mimeType: 'image/jpeg' } }
      ];
      ...
      console.error(error);
      toast.error('AI Verification failed. Please try again or contact support.');
    } finally {
      setLoading(false);
    }
  };
